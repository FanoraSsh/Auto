...
CEKEXPIRED () {
    ...
    Exp1=$(curl -sS https://raw.githubusercontent.com/FanoraSsh/Example/main/Izin/Ip | grep $MYIP | awk '{print $3}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl -sS https://raw.githubusercontent.com/FanoraSsh/Example/main/Izin/Ip | awk '{print $4}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
